...
def _fourier_names(
    *,
    ...
    - If degree > 1: `data` is Polynomial(include_bias=False).fit(...)
    ...
    if degree and degree > 1:
        ...
        # Fourier.fit intentionally preserves its historical use of the default
        # Polynomial degree (2), regardless of the Fourier degree value.
...
def _bilinear_names(
    *,
    ...
    # Use the basis helper itself because Bilinear historically uses set-difference
    # ordering. Reconstructing the mathematical set in a different way can silently
    # mislabel coefficients even when it produces the same terms.
    combinations = basis_function._get_feature_combinations(
        len(base_cols),
        ylag,
        xlag,
    )
    return [_combo_to_name(c, base_cols) for c in combinations]
...
def _render_polynomial(model, input_names, output_name):
    bf = getattr(model, "basis_function", object())
    ...
    return _polynomial_monomial_names(
        ...
        input_names=input_names,
        output_name=output_name,
        drop_intercept=not bool(getattr(bf, "include_bias", True)),
        model_type=getattr(model, "model_type", "NARMAX"),
    )
